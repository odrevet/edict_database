SELECT
  ...
  STRING_AGG(DISTINCT gloss.content, ',') gloss_group,
  STRING_AGG(DISTINCT pos.name, ',') pos_group,
  STRING_AGG(DISTINCT dial.name, ',') dial_group,
  STRING_AGG(DISTINCT misc.name, ',') misc_group,
  STRING_AGG(DISTINCT field.name, ',') field_group
FROM
  entry
